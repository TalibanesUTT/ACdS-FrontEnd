...
export class AppointmentComponent implements OnInit {
    ...
    switchMap(() =>
      combineLatest([
        ...
        this.selectedStatusCtrl.valueChanges.pipe(
          startWith(null),
          tap((status) => console.log('Selected status:', status))
        ),
        this.searchCtrl.valueChanges.pipe(
          startWith(''),
          tap((searchValue) => console.log('Search value:', searchValue))
        ),
      ]).pipe(
        map(([appointments, status, searchValue]) => {
          const filteredByStatus = status ? appointments.filter((appointment) => appointment.status === status) : appointments;
          if (!searchValue) return filteredByStatus;

          const normalizedValue = searchValue.toLowerCase();
          const result = filteredByStatus.filter((appointment) => this._filterAppointmentByColumnValue(appointment, normalizedValue));
          return result;
        }),
        catchError((error) => {
  ...
  constructor(private roleService: UserRoleService, private service: AppointmentService, private dialog: MatDialog) {}
